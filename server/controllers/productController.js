...
const postOneProduct = async (req, res) => {
    uploadProductImages(req, res, async (err) => {
      ...
      try {
        const { title, description, price, technicalInformation, stockQuantity } = req.body;
        const images = req.files;
  
        if (!title || !description || !price || !technicalInformation || !stockQuantity || images.length === 0) {
...
const updateProduct = async (req, res) => {
    uploadProductImages(req, res, async (err) => {
      ...
      try {
        ...
        if (!title || !description || !price || !technicalInformation || !stockQuantity) {
